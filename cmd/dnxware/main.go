...
import (
	...
	promlogflag "github.com/dnxware/common/promlog/flag"
	"github.com/dnxware/dnxware/config"
	"github.com/dnxware/dnxware/discovery"
	...
	prom_runtime "github.com/dnxware/dnxware/pkg/runtime"
	...
	"github.com/dnxware/dnxware/web"
)

var (
	configSuccess = dnxware.NewGauge(dnxware.GaugeOpts{
		Name: "dnxware_config_last_reload_successful",
		Help: "Whether the last configuration reload attempt was successful.",
	})
	configSuccessTime = dnxware.NewGauge(dnxware.GaugeOpts{
		Name: "dnxware_config_last_reload_success_timestamp_seconds",
...
func init() {
	dnxware.MustRegister(version.NewCollector("dnxware"))
...
func main() {
	...
	cfg := struct {
		...
		notifier: notifier.Options{
			Registerer: dnxware.DefaultRegisterer,
		},
		promlogConfig: promlog.Config{},
	}

	a := kingpin.New(filepath.Base(os.Args[0]), "The dnxware monitoring server")

	a.Version(version.Print("dnxware"))
	...
	a.Flag("web.external-url",
		"The URL under which dnxware is externally reachable (for example, if dnxware is served via a reverse proxy). Used for generating relative and absolute links back to dnxware itself. If the URL has a path portion, it will be used to prefix all HTTP endpoints served by dnxware. If omitted, relevant URL components will be derived automatically.").
		PlaceHolder("<URL>").StringVar(&cfg.dnxwareURL)
	...
	a.Flag("web.page-title", "Document title of dnxware instance.").
		Default("dnxware Time Series Collection and Processing Server").StringVar(&cfg.web.PageTitle)
	...
	a.Flag("rules.alert.for-outage-tolerance", "Max time to tolerate dnxware outage for restoring \"for\" state of alert.").
	...
	cfg.web.ExternalURL, err = computeExternalURL(cfg.dnxwareURL, cfg.web.ListenAddress)
	if err != nil {
		fmt.Fprintln(os.Stderr, errors.Wrapf(err, "parse external URL %q", cfg.dnxwareURL))
		os.Exit(2)
	}
	...
	var (
		...
		remoteStorage = remote.NewStorage(log.With(logger, "component", "remote"), dnxware.DefaultRegisterer, localStorage.StartTime, cfg.localStoragePath, time.Duration(cfg.RemoteFlushDeadline))
	...
	// Exclude kingpin default flags to expose only dnxware ones.
	...
	dnxware.MustRegister(configSuccess)
	dnxware.MustRegister(configSuccessTime)
