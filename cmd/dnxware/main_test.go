...
var promPath string
var promConfig = filepath.Join("..", "..", "documentation", "examples", "dnxware.yml")
...
func TestMain(m *testing.M) {
	...
	promPath = filepath.Join(promPath, "dnxware")
...
// As soon as dnxware starts responding to http request should be able to accept Interrupt signals for a graceful shutdown.
...
Loop:
	...
	if !startedOk {
		t.Errorf("dnxware didn't start in the specified timeout")
		return
	}
	if err := prom.Process.Kill(); err == nil {
		t.Errorf("dnxware didn't shutdown gracefully after sending the Interrupt signal")
		...
		t.Errorf("dnxware exited with an unexpected error:%v", stoppedErr)
	}
}
...
func TestComputeExternalURL(t *testing.T) {
	tests := []struct {
		...
		{
			input: "http://proxy.com/dnxware",
			valid: true,
		},
		{
			input: "'https://url/dnxware'",
			valid: false,
		},
		{
...
func TestFailedStartupExitCode(t *testing.T) {
	...
	if exitError, ok := err.(*exec.ExitError); ok {
		...
		t.Errorf("unable to retrieve the exit status for dnxware: %v", err)
	}
}
...
func TestWALSegmentSizeBounds(t *testing.T) {
	...
	for size, expectedExitStatus := range map[string]int{"9MB": 1, "257MB": 1, "10": 2, "1GB": 1, "12MB": 0} {
		...
		if expectedExitStatus == 0 {
			...
			select {
			case err := <-done:
				t.Errorf("dnxware should be still running: %v", err)
