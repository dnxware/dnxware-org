...
// Package testutil provides helpers to test code using the dnxware package
// of client_golang.
//
...
// own code. Rather than verifying that a dnxware.Counter's value has changed
...
// mock implementations of the dnxware.Counter and dnxware.Registerer
...
// testing custom dnxware.Collector implementations and in particular whole
// exporters, i.e. programs that retrieve telemetry data from a 3rd party source
// and convert it into dnxware metrics.
package testutil
...
import (
	...
	"github.com/dnxware/client_golang/dnxware/internal"
)
...
//
// A clear anti-pattern would be to use a metric type from the dnxware
// package to track values that are also needed for something else than the
// exposition of dnxware metrics. For example, you would like to track the
...
// items in a dnxware.Gauge, as it is then easily available as a metric for
...
// considering dnxware metrics) and then expose the number with a
// dnxware.GaugeFunc.
func ToFloat64(c dnxware.Collector) float64 {
	var (
		m      dnxware.Metric
		mCount int
		mChan  = make(chan dnxware.Metric)
...
func CollectAndCompare(c dnxware.Collector, expected io.Reader, metricNames ...string) error {
...
// it to an expected output read from the provided Reader in the dnxware text
// exposition format. If any metricNames are provided, only metrics with those
// names are compared.
func GatherAndCompare(g dnxware.Gatherer, expected io.Reader, metricNames ...string) error {
	got, err := g.Gather()
	if err != nil {
