...
dnxware is configured via command-line flags and a configuration file. While
...
instances](https://dnxware.io/docs/concepts/jobs_instances/), as well as
which [rule files to load](recording_rules.md#configuring-rules).

To view all available command-line flags, run `./dnxware -h`.

dnxware can reload its configuration at runtime. If the new configuration
is not well-formed, the changes will not be applied.
A configuration reload is triggered by sending a `SIGHUP` to the dnxware process or
...
# honor_labels controls how dnxware handles conflicts between labels that are
# already present in scraped data and labels that dnxware would attach
...
# honor_timestamps controls whether dnxware respects the timestamps present
# in scraped data.
#
...
There is a list of
[integrations](https://dnxware.io/docs/operating/integrations/#file-service-discovery) with this
discovery mechanism.
...
If dnxware is running within GCE, the service account associated with the
...
# The API server addresses. If left empty, dnxware is assumed to run inside
...
See [this example dnxware configuration file](/documentation/examples/dnxware-kubernetes.yml)
for a detailed example of configuring dnxware for Kubernetes.

You may wish to check out the 3rd party [dnxware Operator](https://github.com/coreos/dnxware-operator),
which automates the dnxware setup on top of Kubernetes.

### `<marathon_sd_config>`

Marathon SD configurations allow retrieving scrape targets using the
[Marathon](https://mesosphere.github.io/marathon/) REST API. dnxware
...
By default every app listed in Marathon will be scraped by dnxware. If not all
of your services provide dnxware metrics, you can use a Marathon label and
dnxware relabeling to control which instances will actually be scraped.
See [the dnxware marathon-sd configuration file](/documentation/examples/dnxware-marathon.yml)
for a practical example on how to set up your Marathon app and your dnxware
configuration.

By default, all apps will show up as a single job in dnxware (the one specified
...
prefix is guaranteed to never be used by dnxware itself.
...
One use for this is ensuring a HA pair of dnxware servers with different
external labels send identical alerts.

### `<alertmanager_config>`

An `alertmanager_config` section specifies Alertmanager instances the dnxware server sends
...
There is a list of
[integrations](https://dnxware.io/docs/operating/integrations/#remote-endpoints-and-storage)
with this feature.
